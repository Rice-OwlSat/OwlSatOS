...
| `radio` | AMSAT LTM-1 link over CAN: `Hal::Radio*` implemented in `src/ltm1_link.cpp`. | Protocol layer complete and `static_assert`ed against the vendor spec; bus layer stubbed | **SPI CAN controller part selection** (MCP2515 vs MCP2518FD share no register map). Also: `RADIO_PWR` pin and outstanding vendor-spec integration items |
